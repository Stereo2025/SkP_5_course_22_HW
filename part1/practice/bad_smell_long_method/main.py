...
# 1 - создать возвращающую результат функцию, в неё
# передать: пункт 2, пункт 3(2), вернуть пункт 4(3).
def get_users():
    get_read = _read(csv)
    get_sort = _sort(get_read)
    return _filter(get_sort)


# 2 - создать функцию чтения, вывести в
# лист компрехеншн данные из csv (csv)
def _read(csv):
    return [x.split(';') for x in csv.split('\n')]


# 3 - создать функцию сортировки, сортировать будем
# функцией sorted, через лямбду, по элементу [1] (data)
def _sort(data):
    return sorted(data, key=lambda x: int(x[1]))


# 4 - создать функцию фильтрации по который
# выведем только тех людей, кто старше 10 лет. (data)
def _filter(data):
